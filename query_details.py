...
async def main():
    try:
        ...
        # Using a literal or simple parameter format
        ...
        if incapacidad:
            ...
            # Query associated soportes_documentales
            q2 = f"SELECT nombre_archivo, url_archivo FROM soportes_documentales WHERE incapacidad_id = '{incapacidad_id}'"
            soportes = await conn.fetch(q2)
            
            print("\n--- Associated Soportes Documentales ---")
            if soportes:
                for row in soportes:
                    print(f"Nombre: {row['nombre_archivo']}, URL: {row['url_archivo']}")
            else:
